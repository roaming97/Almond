...
from app import app, bcrypt, settings, tasks
from app.dictionaries import video_sorts
from app.forms import AuthForm, ManualAddForm, QuickAddForm, SearchForm
from app.models import Video


def access_denied():
    return True if settings.private_app and "access" not in session else False
    ...
    if access_denied():
        ...
        if Video.query.count() == 0:
            return render_template(
                "index.html",
                home=True,
                private=settings.private_app,
                prevent=settings.prevent_resend,
        ...
        data = Video.query.order_by(video_sorts.get(s, Video.id)).paginate(
            page=page, per_page=settings.videos_per_page
        )

        if query:
            home = False
            data = (
                Video.query.filter(Video.title.contains(query))
                .order_by(video_sorts.get(s, Video.id))
                .paginate(page=1)
            )
        ...
        return (
            render_index(with_paginator=True)
            if data.total > settings.videos_per_page
            else render_index()
        )
...
def auth():
    if not settings.private_app or "access" in session:
        return redirect(
            url_for("index", page=session["current_page"], sort=session["current_sort"])
        )
    else:
        form = AuthForm()
        if form.validate_on_submit():
